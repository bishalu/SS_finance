...
def ml_cross_val_score(
    ...
    ret_scores = parallel(
        delayed(_score_model)(
            classifier, X, y, train, test, sample_weight_train, #clone(classifier)
...
def _score_model(classifier, X, y, train, test, sample_weight_train, sample_weight_score, scoring, require_proba):
    # pylint: disable=invalid-name
    # pylint: disable=comparison-with-callable
    """
    ...
    #TODO: Handle cases with only one class passed for scoring

    if X.iloc[train, :].empty is False:
        if('keras' in str(type(classifier))):
            fit = classifier.fit(X.iloc[train, :], y.iloc[train], batch_size=4, epochs=10)
            
        elif 'sklearn' in str(type(classifier)):
    ...
    if require_proba:
        try:
            ...
            prob = classifier.evaluate(X.iloc[test, :])
