    ...
    * The CV must be purged and embargoed.
    ...
    for i, (train, test) in enumerate(cv_gen.split(X=X)):
        ...
        try: 
            ...
            fold_metrics_values.loc[i] = None
        ...
        for j in feature_sets:
            ...
            try: 
                ...
                fold_metrics_values.loc[i, j] = None
...
def mean_decrease_accuracy(model, X, y, cv_gen, clustered_subsets=None, sample_weight_train=None,
    ...
    * The CV must be purged and embargoed.
    ...
    for i in range(n_repeat):
        mda_round = _mean_decrease_accuracy_round(model=model, X=X, y=y, cv_gen=cv_gen,
                                                  clustered_subsets=clustered_subsets,
                                                  sample_weight_train=sample_weight_train,
                                                  sample_weight_score=sample_weight_score,
                                                  scoring=scoring, require_proba=require_proba,
                                                  random_state=i)
        feature_importances_df = feature_importances_df.append(mda_round)
    # Return aggregated mean importances over all repeats
    return feature_importances_df.groupby(feature_importances_df.index).agg({'mean': 'mean', 'std': 'mean'})
...
def single_feature_importance(clf, X, y, cv_gen, sample_weight_train=None, sample_weight_score=None, scoring=log_loss,
    ...
    * Like MDA, it can conclude that all features are unimportant, because performance is evaluated via OOS CV.
    ...
    for feat in feature_names:
        ...
        print(feat_cross_val_scores)
        feat_cross_val_scores = feat_cross_val_scores[feat_cross_val_scores != np.array(None)]
